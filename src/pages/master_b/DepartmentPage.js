...
import { MdHighlightOff, MdCheckCircle, MdSearch, MdAutorenew, MdEdit } from 'react-icons/md';
...
class DepartmentPage extends React.Component {
    ...
    handleClose = () => {
        ...
        this.setState({ checked: false, currentDepart: { ...resetCurrentDepart } });

    }
    handleCheckboxChange = (event) => {
        let currentDepart = this.state.currentDepart;
        currentDepart["dept_kosmetikyn"] = (event.target.checked ? "Y" : "N");
        this.setState({ checked: event.target.checked, currentDepart });
    }
    ...
    render() {
        ...
        const renderTodos = currentTodos && currentTodos.map((todo, i) => {
            return <tr key={i}>
                <th scope="row">{todo.dept_code}</th>
                <td>{todo.dept_name}</td>
                {
                    <td>
                        {this.setLokasi(todo.dept_lokasi)}
                    </td>
                }
                {
                    <td>
                        {todo.dept_kosmetikyn}
                    </td>
                }
        ...
        return (
            <Page
                title="Departemen"
                breadcrumbs={[{ name: 'department', active: true }]}
                        ...
                        <Card className="mb-3">
                            ...
                            <CardHeader className="d-flex justify-content-between">
                                ...
                                <Modal
                                    ...
                                    <ModalBody>
                                        <Form>
                                            <FormGroup>
                                                ...
                                                <Input type="textarea" id="unit" onChange={evt => this.updateInputValue(evt, "dept_name")} name="namaunit" placeholder="Nama Departemen (Maksimal 50 Karakter)" />
                                            </FormGroup>
                                            <FormGroup>
                                                <Label>Lokasi</Label>
                                                <Input type="select" autoComplete="off" name="select" value={this.state.value} onChange={evt => this.updateInputValue(evt, "dept_lokasi")}>
                                                    ...
                                                    <option value={1}>Floor Atas</option>
                                                    <option value={2}>Floor Bawah</option>
                                                </Input>
                                            </FormGroup>
                                    ...
                                    <ModalFooter>
                                        ...
                                        <Modal
                                            ...
                                            <ModalFooter>
                                                <Button color="primary" onClick={this.insertMasterDept(this.state.currentDepart)} disabled={loading}>
                                                    {!loading && "Ya"}
                                                    {loading && (
                                                        <MdAutorenew />
                                                    )}
                                ...
                                <Modal
                                    ...
                                    <ModalBody>
                                        <Form>
                                            <FormGroup>
                                                <Label >Nama Departemen</Label>
                                                <Input type="namaunit" value={this.state.editDept && this.state.editDept.dept_name} disabled onChange={evt => this.updateInputValue(evt, "dept_name")} name="namaunit" />
                                            </FormGroup>
                                            <FormGroup>
                                                <Label>Lokasi</Label>
                                                <Input type="select" name="select" onChange={evt => this.updateInputValue(evt, "dept_lokasi")}>
                                                    <option hidden>{this.setLokasi(this.state.editDept && this.state.editDept.dept_lokasi)}</option>
                                                    <option value={0}>-</option>
                                                    <option value={1}>Floor Atas</option>
                                                    <option value={2}>Floor Bawah</option>
                                                </Input>
                                    ...
                                    <ModalFooter>
                                        ...
                                        <Modal
                                            ...
                                            <ModalFooter>
                                                <Button id="btnEdit" color="primary" onClick={(e) => this.updateDept('Y', this.state.editDept, this.state.currentDepart, e)} disabled={loading}>
                                                    {!loading && "Ya"}
                                                    {loading && (
                                                        <MdAutorenew />
                                                    )}
                                ...
                                <Modal
                                    ...
                                    <ModalFooter>
                                        <Button id="btnDeaktif" color="primary" onClick={(e) => this.updateDept('N', this.state.active_deptcode_deactive, this.state.currentDepart, e)} disabled={loading}>
                                            {!loading && "Ya"}
                                            {loading && (
                                                <MdAutorenew />
                                            )}
                                ...
                                <Modal
                                    ...
                                    <ModalFooter>
                                        <Button id="btnAktif" color="primary" onClick={(e) => this.updateDept('Y', this.state.active_deptcode_active, this.state.currentDepart, e)} disabled={loading}>
                                            {!loading && "Ya"}
                                            {loading && (
                                                <MdAutorenew />
                                            )}
                                    ...
                                    <thead>
                                        <tr>
                                            <td colSpan='11' className="text-right" style={{ border: 'none' }}>
                                                <Label style={{ width: '50%', textAlign: 'left' }}>F1:Tambah, F3: Untuk Search</Label>
